...
"$_SS" --skill "setup-gbrain" --model "gpt" --parent-pid "$PPID" \
...
```bash
bun run $GSTACK_ROOT/bin/gstack-memory-ingest.ts --probe
```
...
bulk via `bun run $GSTACK_ROOT/bin/gstack-memory-ingest.ts --bulk --quiet`. Set
...
hook runs `bun run $GSTACK_ROOT/bin/gstack-gbrain-sync.ts --incremental --quiet` on every skill
