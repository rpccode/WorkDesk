...
export const FormattedDocumentPreview: React.FC<FormattedDocumentPreviewProps> = ({
      ...
      if (trimmed.startsWith('# ')) {
        ...
        elements.push(
          ...
          >
            ...
            <div>{renderFormattedInlineText(trimmed.replace(/^[-*]\s+/, ''))}</div>
          </div>
        );
        ...
        elements.push(
          ...
          >
            <span style={{ fontWeight: 700, color: '#1e3a8a', minWidth: '16px' }}>{match ? `${match[1]}.` : '1.'}</span>
            <div>{renderFormattedInlineText(match ? match[2] : trimmed)}</div>
          </div>
        );
      } else {
        // Regular paragraph
  ...
  const renderFormattedInlineText = (text: string) => {
    // Splits text with bold **text** tags
  ...
  return (
    ...
    >
      ...
      <div style={{ flex: 1 }}>{parseMarkdownToElements(markdownContent)}</div>
